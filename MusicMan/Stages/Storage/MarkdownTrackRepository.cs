...
internal sealed class MarkdownTrackRepository : ITrackRepository
{
    ...
    private static async Task<(List<Track> Tracks, bool WasReformatted)> GetTracksFromSingleMarkdownAsync(string filePath)
    {
        try
        {
    ...
    private static (Track Track, string Body) Parse(string fileContent, string? filePath = null)
    {
    ...
    private static string Format(Track track, string body)
    {
