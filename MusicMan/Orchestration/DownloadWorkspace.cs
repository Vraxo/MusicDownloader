...
using MusicMan.Stages.Fetching;
using MusicMan.Stages.Processing;
...
internal sealed class DownloadWorkspace(string albumDir)
{
    ...
    public async Task<bool> DownloadAsync(Track track, bool downloadThumbnail)
    {
        ...
        string ytDlpPath = ExecutableFinder.GetFullPath(
            SettingsManager.Current.YtDlpExe,
            SettingsManager.Current.YtDlpDir);
    ...
    public static async Task<bool> DownloadThumbnailOnlyAsync(Track track, string tempThumbBase)
    {
        List<string> args = [
            "--skip-download",
            "--write-thumbnail",
    ...
    public async Task<bool> ProcessAudioAsync(Track track, string inputFile)
    {
        ...
        string ffmpegPath = ExecutableFinder.GetFullPath(
            SettingsManager.Current.FfmpegExe,
    ...
    {
        return FindFileByExtensions(TempFileBase,
        [
            ".webp",
            ".jpg",
            ".png",
    ...
    public static string? FindDownloadedCover(string baseName)
    {
        return FindFileByExtensions(baseName,
        [
            ".webp",
            ".jpg",
            ".png",
