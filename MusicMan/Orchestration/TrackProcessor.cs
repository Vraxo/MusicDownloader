...
internal sealed class TrackProcessor(Track track, int index, int total, CsvTrackRepository repository)
{
    ...
    private readonly string _albumDir = Path.Combine(
    ...
    public async Task<TrackProcessStatus> ProcessAsync()
    {
        ...
        {
            Log.Warning(
                $"Existing file '{Path.GetFileName(outputFile)}' is corrupted. " +
                $"Deleting and queuing for clean redownload...");
    ...
    private async Task<TrackProcessStatus> HandleExistingFileAsync(string outputFile)
    {
        ...
        Log.Warning(
            $"Failed to update metadata in-place for '{Path.GetFileName(outputFile)}' due to container corruption. " +
            $"Deleting and queuing for a clean download...");
