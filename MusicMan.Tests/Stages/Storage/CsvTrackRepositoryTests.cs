...
namespace MusicMan.Tests.Stages.Storage;
...
public sealed class CsvTrackRepositoryTests
{
    ...
    public void FormatCsvValue_WithSpecialCharacters_WrapsAndEscapesQuotes()
    {
        ...
        CsvTrackRepository.FormatCsvValue("with,comma").Should().Be("""
            "with,comma"
            """);

        CsvTrackRepository.FormatCsvValue("""with"quote""").Should().Be("""
            "with""quote"
            """);

        CsvTrackRepository.FormatCsvValue("""
            with
            newline
            """).Should().Be("\"with\nnewline\"");
    }
